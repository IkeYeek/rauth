...
export const ApiService = {
    makeAuthenticatedApiRequest: async <T>(method: "get" | "post" | "patch" | "delete", uri: string, payload?: object): Promise<T> => {
        ...
        if (await authStore.isAuth()) {
            ...
            try {
                switch (method) {
                    ...
                    case "patch":
                    case "delete":
                        res = await method_binding[method as "post" | "patch" | "delete"](envStore.app_base + uri, payload, {
                            validateStatus: s => s < 500,
                            headers: {
                                Authorization: `Bearer ${authStore.getToken()}`
                            }
